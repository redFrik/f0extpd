...
void *f0ext_new(t_symbol *s, int argc, t_atom *argv) {
    ...
    switch (argc) {
        ...
        case 1:
            ...
            break;
        case 3:
            x->valRight = atom_getfloat(argv + 2);
        case 2:
            x->setMin = atom_getfloat(argv);
...
void f0ext_theFunction(t_f0ext *x) {
    x->max = x->max - x->valRight;
    x->min = x->min + x->valRight;
    if (x->valLeft > x->max) {
        x->max = x->valLeft;
    }
    if (x->valLeft < x->min) {
...
void f0_range2_setup(void) {
    f0ext_class = class_new(gensym("f0_range2"), (t_newmethod)f0ext_new, 0, sizeof(t_f0ext), 0, A_GIMME, 0);
    ...
    post("f0_range2 v3.0.0; distributed under GNU GPL license");
}
